...
globals[
  ...
  cum-damaged-crops-given-mit_1
  cum-damaged-crops-given-mit_2
  cum-damaged-crops-given-mit_3
...
;currently used products;
        ...
        adv_measurement_effectivity_used ; turunan dari ME
          eff-aux
          effectivity-mit_1
          eff_at_t  ; auxiliary variable of proportion mitigation implemented succeeed to deter elephant attacks at time t
          ineffectivity_at_t ; auxiliary variable of proportion mitigation implemented fails to deter elephants attacks at time t
...
to import-simulated-product-info
  ...
  file-open "../Data/mitigasi/product_infov4.csv"
...
let product_key 1
  ...
  [
    ...
    [
      ...
      set adv_measurement_effectivity_used item 0 [adv_effectivity] of mmedia with [product_advertised = product_key]
...
let proportion-1 efence-innitial-adopter
let proportion-2 spotlight-innitial-adopter
let proportion-3 carbide-innitial-adopter
...
to setup
 ...
 ;------IO configuration------------------------------------------------------------------------------;
  ...
  ;----------------------------------------------------------------------------------------------------;
  ;----------------------------------------------------------------------------------------------------;
 ;------Behaviour Search Code-------------------------------------------------------------------------;
  declare-var-type-for-behaviour-search
  ingest-data-for-calibration
...
to go
  ...
  adoption-action2
   ...
   ;----------------------------------------------------------------------------------------------------;
   ;-----RECORD OUTPUT----------------------------------------------------------------------------------;
     record-damaged-patch-given-mit-used
    record-mit-data-for-behaviour-search

  if ticks > 15 and ticks < 26 [record-farmers-demography-end_per_tick]

  if ticks = 29 [set final_rmse_score behaviour-search mit1_data mit1_valid mit2_data mit2_valid mit3_data mit3_valid]
   ;----------------------------------------------------------------------------------------------------;
   ;----------------------------------------------------------------------------------------------------;
  tick
end

;----------------------------------------------GO FOR BEHAVIOURSEARCH--------------------------------;
;----------------------------------------------------------------------------------------------------;
to go-bs
  while [ticks < 30]
 [
  ...
  set final_rmse_score behaviour-search mit1_data mit1_valid mit2_data mit2_valid mit3_data mit3_valid
  stop
  ;set final_rmse_score behaviour-search mit1_data mit1_valid mit2_data mit2_valid mit3_data mit3_valid
end

;-----------------------------END OF GO FOR BEHAVIOURSEARCH -----------------------------------------;
;----------------------------------------------------------------------------------------------------;
        ...
        foreach sort (my_firstD_neigh)  ;revisit
          [x ->
            if [alternatives1_i]  of x = product_key [
              ifelse [encounter-hec-with-mit] of x > 0
              [set measurement-effectivity lput  [effectivity-mit_1 ] of x  measurement-effectivity]
              [set measurement-effectivity lput  [adv_measurement_effectivity_used ] of x  measurement-effectivity]
      ...
      [
        table:put SE_dummy product_key
            (
                  ...
                  table:get s-product_knowledge_i product_key * item (product_key - 1) coefSE_5
  ...
  if hold-decision?  = False [
    ...
    ;(2) choosing which mitigation should be placed (apakah di mit_1, mit_2, atau mit_3)
          if alternatives1_i = 0
              ...
              reset-product-eff-when-switching ;to reset their perceived effectiveness value
...
to reset-product-eff-when-switching
  set encounter-hec-with-mit 0
  set effectivity-mit_1 item 0 [adv_effectivity] of mmedia with [product_advertised = [alternatives1_i] of myself]
  table:put eff-aux "a1-eff" []
...
to record-self_EE ; threat operationalization
    ...
    let att_temp_farmers []
    let dmg_temp_farmers []
    let pm_temp_farmers []
    ...
    [;# adding attacks history of farmers
      set total-encounter-hec total-encounter-hec + 1
      set encounter-hec-with-mit encounter-hec-with-mit + 1
      print length sort [land-owned with [attacked? = 1]] of self
      foreach (sort [land-owned with [attacked? = 1]] of self )
    [x -> ask x [

        set attacked-list-at_t_dummy lput [attacked?] of x attacked-list-at_t_dummy
...
to record-bayesian-epidemical-evidence
  if ticks > 1 [
    ask farmers[set t-epidemical-evidence operate-bnlearn EE-from-neighbor t-epidemical-evidence]
  ]
end
...
to record-mitigation-effectivity ;reassess for single adoption
  ask farmers [
    ...
    [
      ...
      set score_2 count land-owned with [last table:get mit_1-effect "residual" > 0] / denumerator
      ;print self print score_1
      ;print self print score_2
       ]
    [
      set score_1 "NaN"
      ;print scores
    ...
    set eff_at_t score_1
    set ineffectivity_at_t score_2
...
to calculate-effectivity-mitigation-used ;reassess for single adoption
  ask farmers [
    ...
    set effectivity-mit_1 py:runresult "calc_eff(var1_aux)"
...
;note --> behaviour repurchasenya :
  ...
  ask farmers with [alternatives1_i != 0][
      ...
      [(ask land-owned [
        set actual_effectivity (1 - eff-broken-factor) * [adv_effectivity] of media ([alternatives1_i] of myself - 1)
        set functionality 2])]
    ]
...
;##############################################OUTPUT PROCEDURE###########################################################
;#
to record-damaged-patch-given-mit-used
  set cum-damaged-crops-given-mit_1 cum-damaged-crops-given-mit_1 + count farmers with [alternatives1_i = 1 and ineffectivity_at_t > 0]
  set cum-damaged-crops-given-mit_2 cum-damaged-crops-given-mit_2 + count farmers with [alternatives1_i = 2 and ineffectivity_at_t > 0]
  set cum-damaged-crops-given-mit_3 cum-damaged-crops-given-mit_3 + count farmers with [alternatives1_i = 3 and ineffectivity_at_t > 0]
end
...
to record-farmers-demography-end
  ...
  "     filename = f'../Model_output/output_from_{start_str}_to_{end_str}.csv'"
  "     with open(filename, 'a', newline='') as file:"
  "          writer = csv.writer(file)"
  "          writer.writerow([datetime.now()] + k)"
  )
  foreach (sort farmers) [ x ->
    ask x [let land-dmg count land-owned with [crops-condition = 0] / count land-owned
      let prone-to-attack [visited-probability-from-raster-data] of patch-here
      let k (list who alternatives1_i capital_assets t-perceived-economical-lossess land-dmg prone-to-attack)
      py:set "recordset" k
      (py:run "write_to_csv(recordset)")
    ]
  ]
end

to record-farmers-demography-end_per_tick
  ...
  "     end_time = datetime.now()"
  "     end_str = end_time.strftime('%Y-%m-%d_%H-%M-%S')"
  "     filename = f'../Model_output/test_switching_from_{start_str}_to_{end_str}.csv'"
  "     with open(filename, 'a', newline='') as file:"
  "          writer = csv.writer(file)"
  "          writer.writerow([datetime.now()] + k)"
  )
  foreach (sort farmers) [ x ->
    ask x [
      let k (list alternatives1_i effectivity-mit_1 actual_effectivity )
      py:set "recordset" k
      (py:run "write_to_csv(recordset)")
    ]
  ]
...
to-report my-neighbor-EE  ;THREAT APPRAISALS
  ...
  table:put table "attacks" last-x-values (flatten_list (flatten_list att_list_dummy)) (attacks-memory-len * count my_firstD_neigh)
  table:put table "damaged?" last-x-values (flatten_list (flatten_list dmg_list_dummy)) (attacks-memory-len * count my_firstD_neigh)
  table:put table "using-pm?" last-x-values (flatten_list (flatten_list pm_list_dummy)) (attacks-memory-len * count my_firstD_neigh)
...
to-report last-x-values [input-list x]
    let result []
    ifelse length input-list > x [
        set result sublist input-list (length input-list - x) length input-list
    ] [
        set result input-list
    ]
    report result
end
...
to-report operate-bnlearn [EE-tables t-epp-eff]  ;THREAT APPRAISALS

  py:set "data" table:to-list EE-tables
  py:set "previous_eff" t-epp-eff
  ...
  (py:run
    ...
    "#inferencing"
    "     try:"
    "          q1 = bn.inference.fit(model_mle, variables=['damaged?'], evidence={'attacks':1,'using-pm?':1},verbose=0)"
    "     except:"
  ...
  (py:run
    ...
    "except:"
    "     result = 0"
    "     print('using previous eff')"
  )
  report py:runresult "result"
end

to-report manual-bn [EE-tables]
  py:set "att_l" table:get EE-tables "attacks"
  py:set "dmg_l" table:get EE-tables "damaged?"
  py:run "att_l_float = [float(x) for x in att_l]"
  py:run "dmg_l_float = [float(x) for x in dmg_l]"
  py:run "print(att_l_float)"

  (py:run
    "import numpy as np"

  "def conditional_probability(list_att, list_dmg):"
    "     att_array = np.array(list_att)"
    "     dmg_array = np.array(list_dmg)"
    "     joint_probability = np.sum((att_array == 1) & (dmg_array == 1)) / len(att_array)"
    "     probability_att = np.sum(att_array == 1) / len(att_array)"
    "     probability = joint_probability / probability_att"
    "     return probability"

  )
  report py:runresult "conditional_probability(att_l_float,dmg_l_float)"
end


to-report manual-bn-using-pm2 [EE-tables]
     py:set "att_l" table:get EE-tables "attacks"
    py:set "dmg_l" table:get EE-tables "damaged?"
    py:set "pm_l" table:get EE-tables "using-pm?"
  (py:run
"import numpy as np"
    "print(type (att_l[4]))"
"def calculate_inference(attacks_l, damage_l, using_pm_l):"
"     attacks_array = np.array(attacks_l)"
"     damage_array = np.array(damage_l)"
"     using_pm_array = np.array(using_pm_l)"
"     try:"
"          result = np.sum((damage_array == 1) & (attacks_array == 1) & (using_pm_array == 0)) / np.sum((attacks_array == 1) & (using_pm_array == 0))"
"     except ZeroDivisionError:"
"          #print(ZeroDivisionError)"
"          joint_probability = np.sum((attacks_array == 1) & (damage_array == 1)) / len(attacks_array)"
"          probability_att = np.sum(attacks_array == 1) / len(attacks_array)"
"          result = joint_probability / probability_att"
"     return result"

)
  report py:runresult "calculate_inference(att_l,dmg_l,pm_l)"
end

to-report manual-bn-using-pm-used [EE-tables prev-t-epp-eff]

  py:set "att_l" table:get EE-tables "attacks"
  py:set "dmg_l" table:get EE-tables "damaged?"
  py:set "pm_l" table:get EE-tables "using-pm?"
  py:set "prev-value" prev-t-epp-eff
  (py:run
    "import numpy as np"

    "def calculate_inference(attacks_l, damage_l, prev-value = prev-value, using_pm_l=None):"
    "  attacks_array = np.array(attacks_l)"
    "  damage_array = np.array(damage_l)"
    "  if using_pm_l is not None:"
    "    using_pm_array = np.array(using_pm_l)"
    "    denominator = np.sum((attacks_array == 1) & (using_pm_array == 0))"
    "    if denominator != 0:"
    "      result = np.sum((damage_array == 1) & (attacks_array == 1) & (using_pm_array == 0)) / denominator"
    "    else:"
    "      result = prev-value"
    "  else:"
    "    denominator = np.sum(attacks_array == 1)"
    "    if denominator != 0:"
    "      result = np.sum((damage_array == 1) & (attacks_array == 1)) / denominator"
    "    else:"
    "      result = prev-value"
    "  return result"
  )
...
to-report return-patch-set-to-be-attacked_v2  [number-to-be-called]
  ...
  if k < 0 [set k 0]
  let r max-n-of k patches with [is-boundary-patches? = True and crops-condition > 0] [ (visited-probability-from-raster-data - rng-visited-chance)]
  report  r
end
...
to-report value-mapper [values data-type]
 ...
 ;operasionalisasinya dilihat data typenya kemudian menggunakan if statement untuk setiap tipe data yang berbeda
  ...
  let s-product_knowledge_bins [0 .1 .8 .90 1000]
 ...
 ;;############################################RESPONSE COST VARIABLE###############################################################;
  ...
  [
    ...
    set results py:runresult "(1 - map_value(value, bins_used, mapper))"
  ]
...
;####################################################BEHAVIOUR SEARCH REPORT PROCEDURE IN NETLOGO #########################################################;

to declare-var-type-for-behaviour-search
  set mit1_data []
  set mit2_data []
  set mit3_data []
end
to record-mit-data-for-behaviour-search
  if ticks mod 4 = 0 [
  set mit1_data lput cum-damaged-crops-given-mit_1 mit1_data
  set mit2_data lput cum-damaged-crops-given-mit_2 mit2_data
    set mit3_data lput cum-damaged-crops-given-mit_3 mit3_data ]
end

to ingest-data-for-calibration
  set mit1_valid []
  set mit2_valid []
  set mit3_valid []
  file-open "../Data/validation_data/valid_data_norm_cum.csv"
  let header  csv:from-row file-read-line
  while [not file-at-end?]
  [ let data csv:from-row file-read-line
    set mit1_valid lput item 0 data mit1_valid
    set mit2_valid lput item 1 data mit2_valid
    set mit3_valid lput item 2 data mit3_valid ]
  file-close
end
to-report behaviour-search [mit1_mod mit1_act mit2_mod mit2_act mit3_mod mit3_act ]
  ;input Declaration
  py:set "list_mit1_mod" mit1_mod
  py:set "list_mit1_act" mit1_act

  py:set "list_mit2_mod" mit2_mod
  py:set "list_mit2_act" mit2_act

  py:set "list_mit3_mod" mit3_mod
  py:set "list_mit3_act" mit3_act
  ;Function Declaration
  (py:run
   "def rmse(arr_act, arr_mod) -> float:"
    "     import numpy as np"
    "     from math import sqrt"
    "     mse = ((arr_act - arr_mod) ** 2).mean()"
    "     rmse = sqrt(mse)"
    "     return rmse"
    )

  (py:run
    "def standardize_list(lst: list) -> np.ndarray:"
    "     import numpy as np"
  ...
  (py:run
    "def calc_rsme_for_netlogo(mit1_mod,mit1_act,mit2_mod,mit2_act,mit3_mod,mit3_act):"
    "     rmse1 = rmse(standardize_list(mit1_act),standardize_list(mit1_mod))"
    "     rmse2 = rmse(standardize_list(mit2_act),standardize_list(mit2_mod))"
    "     rmse3 = rmse(standardize_list(mit3_act),standardize_list(mit3_mod))"

    "     rmse_all = rmse1 + rmse2 + rmse3"
    "     return rmse_all"
    )

  report py:runresult "calc_rsme_for_netlogo(list_mit1_mod,list_mit1_act,list_mit2_mod,list_mit2_act,list_mit3_mod,list_mit3_act)"
end

;#################################################### END OF BEHAVIOUR SEARCH REPORT PROCEDURE IN NETLOGO ##################################################;
@#$#@#$#@
GRAPHICS-WINDOW
205
54
...
"" ""
PENS
"1-sawit" 1.0 0 -7500403 true "" "plot count farmers with [alternatives1_i = 1 and [crop-type] of one-of land-owned = \"sawit\"]"
...
"crops-condition" 1.0 0 -6459832 true "" "plot mean [crops-condition] of patches with [land_type = \"agriculture\"] / 100 "
...
107
eff
mean [effectivity-mit_1] of farmers with [total-encounter-hec > 0]
3
1
11
...
"" ""
PENS
"general" 1.0 0 -16777216 true "" "plot mean [effectivity-mit_1] of farmers with [encounter-hec-with-mit > 0]"
"e-fence" 1.0 0 -11221820 true "" "plot mean [effectivity-mit_1] of farmers with [encounter-hec-with-mit > 0 and alternatives1_i = 1]"
"spotlight" 1.0 0 -2674135 true "" "plot mean [effectivity-mit_1] of farmers with [encounter-hec-with-mit > 0 and alternatives1_i = 2]"
"carbide" 1.0 0 -4079321 true "" "plot mean [effectivity-mit_1] of farmers with [encounter-hec-with-mit > 0 and alternatives1_i = 3]"
...
"" ""
PENS
"1" 1.0 0 -16777216 true "" "plot count farmers with [alternatives1_i = 1 and ineffectivity_at_t > 0] "
"2" 1.0 0 -2674135 true "" "plot count farmers with [alternatives1_i = 2 and ineffectivity_at_t > 0] "
"3" 1.0 0 -11221820 true "" "plot count farmers with [alternatives1_i = 3 and ineffectivity_at_t > 0] "
"1-cum" 1.0 0 -7500403 true "" "plot cum-damaged-crops-given-mit_1 "
"2-cum" 1.0 0 -955883 true "" "plot cum-damaged-crops-given-mit_2 "
"3-cum" 1.0 0 -6459832 true "" "plot cum-damaged-crops-given-mit_3 "
...
"" ""
PENS
"1" 1.0 0 -16777216 true "" "plot mean [effectivity-mit_1] of farmers with [alternatives1_i = 1]"
"2" 1.0 0 -7500403 true "" "plot mean [effectivity-mit_1] of farmers with [alternatives1_i = 2]"
"1-enc" 1.0 0 -955883 true "" "plot mean [effectivity-mit_1] of farmers with [alternatives1_i = 1 and encounter-hec-with-mit > 0]"
"2-enc" 1.0 0 -6459832 true "" "plot mean [effectivity-mit_1] of farmers with [alternatives1_i = 2 and encounter-hec-with-mit > 0]"
...
"" ""
PENS
"default" 1.0 0 -2674135 true "" "plot standard-deviation [effectivity-mit_1] of farmers with [alternatives1_i = 1 and encounter-hec-with-mit > 0]"
"pen-1" 1.0 0 -7500403 true "" "plot standard-deviation [effectivity-mit_1] of farmers with [alternatives1_i = 2 and encounter-hec-with-mit > 0]"
"pen-2" 1.0 0 -955883 true "" "plot count patches with [land_type = \"agriculture\" and mitigation-used = 1 and functionality = 2]  "
...
154
std-ee
standard-deviation [t-epidemical-evidence] of farmers
1
1
11
...
<experiments>
  ...
  <experiment name="test-run 8 x base scenario" repetitions="4" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="40"/>
    <metric>mean [threat-appraisals] of farmers</metric>
    ...
    <enumeratedValueSet variable="top-down-success-reach-treshold">
      <value value="0.65"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="TA2">
      <value value="0.3"/>
  ...
  <experiment name="test-run 4x base scenario v2" repetitions="8" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="40"/>
    <metric>mean [threat-appraisals] of farmers</metric>
  ...
  <experiment name="test-run 4x base scenario v3" repetitions="8" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="40"/>
    <metric>mean [threat-appraisals] of farmers</metric>
  ...
  <experiment name="test-run 4x base scenario v4" repetitions="8" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="30"/>
    <metric>mean [threat-appraisals] of farmers</metric>
    ...
    <metric>count patches with [land_type = "agriculture" and mitigation-used = 3 and functionality = 1] / (count patches with [land_type = "agriculture" and mitigation-used = 3 ] + 1)</metric>
    <metric>count patches with [land_type = "agriculture" and functionality = 1] / count patches with [land_type = "agriculture" ]</metric>
    <metric>cum-damaged-crops-given-mit_1</metric>
    <metric>cum-damaged-crops-given-mit_2</metric>
    <metric>cum-damaged-crops-given-mit_3</metric>
    <enumeratedValueSet variable="between-adoption-time-delay">
      <value value="8"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="farmers-population">
  ...
  <experiment name="parameterization" repetitions="3" runMetricsEveryStep="true">
    ...
    <enumeratedValueSet variable="eff-broken-factor">
      <value value="0"/>
      <value value="0.25"/>
      <value value="0.5"/>
      <value value="0.75"/>
    ...
    <steppedValueSet variable="carbide-innitial-adopter" first="0.1" step="0.1" last="0.3"/>
    <enumeratedValueSet variable="farmers-population">
      <value value="100"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="trustworthiness-treshold">
    ...
    <steppedValueSet variable="spotlight-innitial-adopter" first="0.2" step="0.05" last="0.5"/>
    <enumeratedValueSet variable="std-wealth">
      <value value="1000000"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="TA3">
    ...
    <steppedValueSet variable="attacks-memory-len" first="10" step="10" last="30"/>
    <enumeratedValueSet variable="scenario">
      <value value="&quot;Baseline&quot;"/>
    </enumeratedValueSet>
    <steppedValueSet variable="mean-attack" first="5" step="10" last="40"/>
    <enumeratedValueSet variable="SE1">
      <value value="0.3"/>
    </enumeratedValueSet>
  </experiment>
  <experiment name="parameterization_innitial_adopter" repetitions="2" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <final>show final_rmse_score</final>
    <timeLimit steps="30"/>
    <enumeratedValueSet variable="between-adoption-time-delay">
      <value value="8"/>
    </enumeratedValueSet>
    ...
    <enumeratedValueSet variable="carbide-innitial-adopter">
      <value value="0.1"/>
      <value value="0.25"/>
      <value value="0.3"/>
      <value value="0.4"/>
    ...
    <enumeratedValueSet variable="spotlight-innitial-adopter">
      <value value="0.1"/>
      <value value="0.2"/>
      <value value="0.3"/>
    </enumeratedValueSet>
  ...
  <experiment name="calibration v3" repetitions="20" runMetricsEveryStep="true">
    <setup>setup-bs</setup>
    <go>go-bs</go>
    <timeLimit steps="30"/>
    <metric>mean [threat-appraisals] of farmers</metric>
    ...
    <metric>cum-damaged-crops-given-mit_2</metric>
    <metric>cum-damaged-crops-given-mit_3</metric>
    <metric>final_rmse_score</metric>
    <enumeratedValueSet variable="between-adoption-time-delay">
      <value value="8"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="eff-broken-factor">
    ...
    <enumeratedValueSet variable="efence-innitial-adopter">
      <value value="0.05"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="SE2">
      <value value="0.3"/>
  ...
  <experiment name="calibration v4" repetitions="3" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="30"/>
    <metric>mean [threat-appraisals] of farmers</metric>
  ...
  <experiment name="attack variation v5" repetitions="4" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="80"/>
    <metric>mean [threat-appraisals] of farmers</metric>
    ...
    <metric>cum-damaged-crops-given-mit_2</metric>
    <metric>cum-damaged-crops-given-mit_3</metric>
    <metric>mean [effectivity-mit_1] of farmers with [total-encounter-hec &gt; 0]</metric>
    <enumeratedValueSet variable="between-adoption-time-delay">
      <value value="8"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="eff-broken-factor">
  ...
  <experiment name="calibration v5" repetitions="5" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="40"/>
    <metric>mean [threat-appraisals] of farmers</metric>
  ...
  <experiment name="communication variation v5" repetitions="5" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="40"/>
    <metric>mean [threat-appraisals] of farmers</metric>
  ...
  <experiment name="communication variation v5c" repetitions="4" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="80"/>
    <metric>mean [threat-appraisals] of farmers</metric>
  ...
  <experiment name="capital variation v5" repetitions="5" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="40"/>
    <metric>mean [threat-appraisals] of farmers</metric>
  ...
  <experiment name="capital variation v6" repetitions="1" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="80"/>
    <metric>mean [threat-appraisals] of farmers</metric>
  ...
  <experiment name="force recalibrate" repetitions="5" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="40"/>
    <metric>mean [threat-appraisals] of farmers</metric>
  ...
  <experiment name="baseline v5" repetitions="3" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="80"/>
    <metric>mean [threat-appraisals] of farmers</metric>
  ...
  <experiment name="perceived_eff_v1" repetitions="1" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="25"/>
    <metric>mean [threat-appraisals] of farmers</metric>
    ...
    <metric>cum-damaged-crops-given-mit_2</metric>
    <metric>cum-damaged-crops-given-mit_3</metric>
    <metric>[effectivity-mit_1] of farmers with [encounter-hec-with-mit &gt; 0 and alternatives1_i = 1]</metric>
    <metric>[effectivity-mit_1] of farmers with [encounter-hec-with-mit &gt; 0 and alternatives1_i = 2]</metric>
    <metric>final_rmse_score</metric>
    <enumeratedValueSet variable="between-adoption-time-delay">
      <value value="8"/>
    </enumeratedValueSet>
  ...
  <experiment name="attack variation v6" repetitions="1" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="80"/>
    <metric>mean [threat-appraisals] of farmers</metric>
  ...
  <experiment name="capital variation v7" repetitions="1" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="80"/>
    <metric>mean [threat-appraisals] of farmers</metric>
