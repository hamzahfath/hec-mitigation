...
to import-simulated-product-info
  ...
  file-open "D:/Onedrive/Skripsi/Thesis/Netlogo Code/product_info.csv"
...
to import-plant-commodity-db
  ...
  file-close-all
  file-open "D:/Onedrive/Skripsi/Thesis/Netlogo Code/data_tumbuhan_test_v5_Putri_marampa.csv"
  let header csv:from-row file-read-line
...
to test-upload-peta
  ...
  set peta-dasar gis:load-dataset "D:/Onedrive/Skripsi/Thesis/Netlogo Code/RESOURCE PETA/Muara Sekalo Suo Suo - peta dasar_v1.asc"
  ...
  set probability-map-data gis:load-dataset "D:/Onedrive/Skripsi/Thesis/Netlogo Code/RESOURCE PETA/Muara Sekalo Suo Suo - raster probability map_v1.asc"
...
ask n-of (farmers-population * proportion-0) farmers [ set alternatives1_i 0 ]
...
create-farmers 1 [
     ...
     ; setxy item 0 row item 1 row
      while [any? other farmers in-radius 10 and not member? [desa] of patches in-radius 4 member-desa ]
...
to generate-crops ;mitigation-used perlu ditambah di prosedur go
  ...
  ask farmers [
    let k patches in-radius 2 with [ownership = 0 and desa <= 10 ]
...
to perform-SA
  ...
  (py:run
"import os, glob"
"for f in glob.glob('sensitivity_analysis/*.txt'):"
"     os.remove(f)")
  ; 'attacks-memory-len',
   ;               'average-node-degree',
  ...
  foreach params-master-list [ params-set ->
    ...
    ;#create a new file
    let filename (word "sensitivity_analysis/reach_loops_" run-number ".txt")
...
;-----------------------------------------IF already made previous purchases----------------------------------------------------;
    ;(1) choosing candidate that hasn't been adopted with considering their used tools
...
;  ask patches with [ any? turtles-here ] [
 ;   set turtle-here 1
  ...
  gis:store-dataset ras_out "./model_outputs/count_attacked.asc"

  let ras_out_2 gis:patch-dataset mitigation-used
  gis:store-dataset ras_out_2 "./model_outputs/mitigation_used.asc"
end
...
to-report morris-params-gen
  ...
  (py:run
    ...
    "     params_csv.to_csv('./sensitivity_analysis/morris_params_csv.csv')"
    "     return param_values" )
