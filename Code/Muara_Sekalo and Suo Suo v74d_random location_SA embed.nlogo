...
to-report operate-bnlearn [EE-tables t-epp-eff]  ;THREAT APPRAISALS
  ...
  (py:run
    ...
    "#inferencing"
    "     try:"
    "          q1 = bn.inference.fit(model_mle, variables=['damaged?'], evidence={'attacks':1,'damaged?':1},verbose=0)"
    "     except:"
...
to-report manual-bn-using-pm2 [EE-tables t-epp-eff]
     py:set "att_l" table:get EE-tables "attacks"
    py:set "dmg_l" table:get EE-tables "damaged?"
...
"     try:"
"          result = np.sum((damage_array == 1) & (attacks_array == 1) & (using_pm_array == 1)) / np.sum((attacks_array == 1) & (using_pm_array == 1))"
